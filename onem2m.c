...
#define DB_NAME "RESOURCE.db"

extern ResourceTree *rt;

int delete_child_cin(RTNode* rtnode);
...
int db_delete_child_cin(RTNode* rtnode)
{
	...
	while ((ret = dbcp->get(dbcp, &key, &data, DB_NEXT)) == 0) {
    	...
    	if (result != NULL) 
		{
        	fprintf(stderr, "Deleting CNT %s's Child CIN %s\n", ri, (char*)key.data);
			ret = dbcp->del(dbcp, 0);
            if (ret != 0) {
                fprintf(stderr, "Failed to delete the key-value pair: %s\n", db_strerror(ret));
                break;
            }
    	} 
		else 
		{
        	fprintf(stderr, "The search string was not found in the long string.\n");
    	}
        // Clear the DBT data for the next iteration
        memset(&key, 0, sizeof(key));
        memset(&data, 0, sizeof(data));
	}
...
int delete_rtnode_and_db_data(oneM2MPrimitive *o2pt, RTNode *rtnode, int flag) {
	...
	case RT_AE : 
	case RT_CNT : 
		fprintf(stderr, "Delete CNT's Child CINs...\n");
